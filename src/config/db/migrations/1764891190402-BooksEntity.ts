...
export class BooksEntity1764891190402 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."books_source_enum" AS ENUM('GOOGLE_BOOKS_API', 'OPEN_LIBRARY_API')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_books_externalId_source" ON "books" ("external_id", "source") `,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX "public"."idx_books_externalId_source"`,
    );
