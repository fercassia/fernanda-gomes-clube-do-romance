...
export class LoginAttemptGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (attempts && attempts >= this.MAX_ATTEMPTS) {
      const ttl = this.loginAttemptService.convertTtlToMinutes(
        await this.loginAttemptService.getTtl(ip),
      );
      Logger.error(
        `${HttpStatus.TOO_MANY_REQUESTS} - IP ${ip} has been blocked - LoginAttemptGuard`,
      );
