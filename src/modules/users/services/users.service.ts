...
} from '../interfaces/repository/iUsersRepository.interface';
...
export class UsersService {
  ...
  async create(userModel: UsersModel): Promise<CreateUsersResponseDto> {
    const userExist: UsersEntity | null =
      await this.usersRepository.findByEmailOrDisplayName(
    ...
    if (userExist) {
      Logger.warn(
        `${HttpStatus.CONFLICT} - (${userModel.email} - ${userExist.email}) or (${userModel.displayName} - ${userExist.displayName}) are equal.`,
        Metadata.create({ serviceMethod: 'UsersService.create' }),
      );
