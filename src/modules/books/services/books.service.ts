...
import {
  BOOKS_REPOSITORY_INTERFACE,
  type IBooksRepositoryInterface,
} from '../interfaces/repository/iBooksRepository.interface';
...
export class BooksService {
  constructor(
    @Inject(BOOKS_REPOSITORY_INTERFACE)
    private readonly booksRepository: IBooksRepositoryInterface,
  ...
  async getBooks(query: BooksSearchModel) {
    if (query.query === null) {
      Logger.warn(
        `Bad Request - At least one search parameter must be provided.`,
        Metadata.create({ serviceMethod: 'BooksService.getBooks' }),
      );
    ...
    if (booksFound === null) {
      ...
      const newBooksSavedReturned: BooksEntity[] | null =
        await this.verifyExistenceBooks(query);
  ...
  private async callExternalApi(
    query: string,
  ): Promise<GoogleBooksApiResponseDto> {
    const returnGoogle =
      await this.googleBooksApiService.getBooksGoogleApi(query);
    return returnGoogle;
  }
  ...
  private async filterBooksDifference(
    booksExternal: BooksModel[],
    ...
    const booksDifference = booksExternal.filter((book) => {
      return !existBooks.some(
        (existBook) =>
          existBook.externalId === book.externalId &&
  ...
  private async verifyExistenceBooks(
    query: BooksSearchModel,
