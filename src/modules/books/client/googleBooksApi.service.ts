...
export class GoogleBooksApiService {
  ...
  constructor(
    ...
    private readonly googleBooksApiMapper: GoogleBooksApiMapper,
  ) {}
  ...
  async getBooksGoogleApi(query: string): Promise<GoogleBooksApiResponseDto> {
    ...
    const booksTitleFiltered: IApiGoogleBooksItem[] =
      this.removeBooksWithoutTitles(data);
    return this.googleBooksApiMapper.toResponseDtoApiGoogleBooks(
      booksTitleFiltered,
    );
  }
